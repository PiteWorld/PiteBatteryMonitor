...
public class MyLineChartView extends LineChartView {
	...
	protected void onDraw(Canvas arg0) {
		...
		int index = 0;
		int gap = 0;
		if (TrendFragment.list != null) {
			index = gap = TrendFragment.list.getDataInEveryPage().getBatteryByPage().getData().size() / 5;
		}
		for (float xline = 0; xline < w; xline += (w / 5)) { // Y轴平行线
			...
			if (TrendFragment.list != null) {
				arg0.drawText(setSulist(
						TrendFragment.list.getDataInEveryPage().getBatteryByPage().getData().get(index).getTime()),
						xline - 50, h, pa1);
			}
			index += gap;
		}
