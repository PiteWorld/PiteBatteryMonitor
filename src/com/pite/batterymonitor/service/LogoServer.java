...
public class LogoServer extends IntentService{
	...
	private void DownApk(Intent intent){
		...
		if(intent!=null&&intent.getStringExtra("flage").equals("1")){
			...
			try {
			 ...
			 try {
				...
				notif.tickerText="通知";
				notif.contentView = new RemoteViews(getPackageName(),R.layout.progressbar_content);
				notif.contentView.setTextViewText(R.id.pro_tv, "0%");
				notif.contentView.setProgressBar(R.id.pro_progressBar,100,0,false);
				//显示进度条
				...
				//拿到服务器的响应吗
					...
					File apk = new File(file+"/"+"PiteBatteryMonitor.apk");
	 ...
	 private void notificationInit(){
		 ...
		 // notif.tickerText=getResources().getString(R.string.loading);
		  //notif.tickerText="kaishi";
